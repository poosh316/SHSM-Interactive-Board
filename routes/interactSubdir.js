...
router.get(/^\/$|^\/drone(\/)?$|^\/drone(.ejs)?$/, async (req, res) => {
    res.render(path.join(__dirname, "..", "views", "interactPages", "drone.ejs"), await sendTime.timeExpiry(req.cookies.int_token));
});

router.get(/^\/lights(\/)?(.ejs)?$/, async (req, res) => {
    console.log("thing: " + (await sendTime.timeExpiry(req.cookies.int_token))["secondsRemaining"]);
    res.render(path.join(__dirname, "..", "views", "interactPages", "lights.ejs"), await sendTime.timeExpiry(req.cookies.int_token));
});
